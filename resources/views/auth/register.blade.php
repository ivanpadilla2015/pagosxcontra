<x-authentication-layout>
    ...
    <form method="POST" action="{{ route('register') }}">
        ...
        <div class="space-y-4">
            ...
            <div>
                <x-label for="regional_id">{{ __('Regional') }} <span class="text-red-500">*</span></x-label>
                <select id="regional_id" name="regional_id" required class="form-select w-full">
                    <option value="">{{ __('Seleccionar...') }}</option>
                    @foreach (\App\Models\Regional::orderBy('name')->get() as $regional)
                        <option value="{{ $regional->id }}" @selected(old('regional_id') == $regional->id)>{{ $regional->name }}</option>
                    @endforeach
                </select>
            </div>
