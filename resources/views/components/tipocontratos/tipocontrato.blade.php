...
/**
 * Componente Volt (Livewire 4) para gestionar el CRUD de "Tipos de Contrato".
 *
...
new class extends Component
{
    ...
    /**
     * ID del tipo de contrato que se está editando.
     * null = modo "crear"; un entero = modo "editar".
     */
    public ?int $editingId = null;

    /** ID del tipo de contrato seleccionado para eliminar (guardado al confirmar). */
    public ?int $tipocontratoToDeleteId = null;

    /** Nombre del tipo de contrato a eliminar, usado solo para mostrarlo en el modal. */
    public string $tipocontratoToDeleteName = '';

    /** Campo del formulario: nombre del tipo de contrato. */
    public string $name = '';
     ...
     *
     * @return array Reglas aplicadas a 'name'.
     */
    ...
    /**
     * Devuelve la lista paginada de tipos de contrato.
     ...
     *
     * @param int|null $id ID del tipo de contrato a editar. Si es null, es modo crear.
     */
    public function openModal(?int $id = null): void
    {
        ...
        // Si recibimos un ID, cargamos los datos del tipo de contrato a editar.
        if ($id) {
            $tipocontrato = Tipocontrato::findOrFail($id);
            $this->name = $tipocontrato->name;
        }
     ...
     * Livewire resuelve el modelo Tipocontrato automáticamente desde el ID
     * que se le pasa en el botón (wire:click="confirmDelete({{ $tipocontrato->id }})").
     *
     * @param Tipocontrato $tipocontrato Modelo inyectado por Livewire.
     */
    public function confirmDelete(Tipocontrato $tipocontrato): void
    {
        $this->tipocontratoToDeleteId = $tipocontrato->id;
        $this->tipocontratoToDeleteName = $tipocontrato->name;
    ...
    public function closeDeleteModal(): void
    {
        ...
        $this->tipocontratoToDeleteId = null;
        $this->tipocontratoToDeleteName = '';
    }

    /**
     * Elimina definitivamente el tipo de contrato seleccionado y muestra un mensaje.
     * Usa findOrFail para lanzar 404 si el ID ya no existe.
     */
    public function delete(): void
    {
        Tipocontrato::findOrFail($this->tipocontratoToDeleteId)->delete();
        session()->flash('message', 'Tipo de contrato eliminado correctamente.');
    ...
    /**
     * Valida el formulario y crea o actualiza el tipo de contrato según corresponda.
    ...
    public function save(): void
    {
        ...
        if ($this->editingId) {
            ...
            Tipocontrato::findOrFail($this->editingId)->update($data);
            session()->flash('message', 'Tipo de contrato actualizado correctamente.');
        } else {
            // Modo creación: inserta un nuevo registro.
            Tipocontrato::create($data);
            session()->flash('message', 'Tipo de contrato creado correctamente.');
        }
...
<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    {{-- Encabezado: título de la página y botón "Nuevo Tipo de Contrato" --}}
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Tipos de Contrato</h1>
        {{-- Abre el modal en modo crear (sin ID) --}}
        <button type="button" wire:click="openModal()" class="btn bg-white hover:bg-gray-100 text-gray-800 border border-gray-200">Nuevo Tipo de Contrato</button>
    </div>
    ...
    {{-- Tabla con la lista paginada de tipos de contrato (usa la propiedad computada $this->tipocontratos) --}}
    <div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl border border-gray-200 dark:border-gray-700/60">
        <table class="table-auto w-full">
            ...
            <tbody class="text-sm divide-y divide-gray-100 dark:divide-gray-700/60">
                {{-- Recorre los tipos de contrato; @empty muestra mensaje si no hay resultados --}}
                @forelse ($this->tipocontratos as $tipocontrato)
                    <tr>
                        <td class="px-4 py-3 text-gray-800 dark:text-gray-100">{{ $tipocontrato->name }}</td>
                        <td class="px-4 py-3 text-right whitespace-nowrap">
                            {{-- Botón editar: abre el modal pasando el ID --}}
                            <button type="button" wire:click="openModal({{ $tipocontrato->id }})" class="text-violet-500 hover:text-violet-600 mr-3" title="Editar">
                            ...
                            <button type="button" wire:click="confirmDelete({{ $tipocontrato->id }})" class="text-rose-500 hover:text-rose-600" title="Eliminar">
                ...
                @empty
                    <tr>
                        <td colspan="2" class="px-4 py-6 text-center text-gray-500 dark:text-gray-400">No hay tipos de contrato registrados.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    @if ($modalOpen)
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60" wire:click="closeModal" wire:key="tipocontrato-modal">
            <div class="w-full max-w-md bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6" wire:click.stop>
                ...
                <h2 class="text-lg font-bold text-gray-800 dark:text-gray-100 mb-4">{{ $editingId ? 'Editar Tipo de Contrato' : 'Nuevo Tipo de Contrato' }}</h2>
    ...
    @if ($deleteModalOpen)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60" wire:click="closeDeleteModal" wire:key="delete-modal">
            <div class="w-full max-w-md bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6" wire:click.stop>
                ...
                <h2 class="text-lg font-bold text-gray-800 dark:text-gray-100 mb-2 text-center">Eliminar Tipo de Contrato</h2>
                <p class="text-sm text-gray-600 dark:text-gray-400 text-center mb-6">¿Estás seguro de que deseas eliminar <span class="font-semibold text-gray-800 dark:text-gray-100">{{ $tipocontratoToDeleteName }}</span>? Esta acción no se puede deshacer.</p>
