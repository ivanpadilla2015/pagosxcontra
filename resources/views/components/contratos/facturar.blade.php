...
new class extends Component
{
    ...
    public function buscarContrato(): void
    {
        ...
        $this->contrato = Contrato::with(['proveedor', 'movirubros.rubro', 'itemcontratos.producto', 'itemcontratos.movirubro', 'itemcontratos.rubro'])
    ...
    public function calcularRetencionesLinea(int $idx): void
    {
        ...
        if ($linea['es_ajuste'] ?? false) {
            ...
            // Línea sencilla (sin itemcontrato): usar producto_id del array
    ...
    // ------------------------------------------------------------------
    // Saldo por rubro (informativo + control)
    // ------------------------------------------------------------------
    ...
    public function grabarFactura(): void
    {
        ...
        // Crear líneas
        foreach ($this->lineas as $idx => $linea) {
            $itemcontrato = Itemcontrato::find($linea['itemcontrato_id'] ?? null);
            ...
            if ($esAjuste) {
                ...
                $facturaLinea = FacturaLinea::create([
                    ...
                    'producto_id'             => $linea['producto_id'] ?? $itemcontrato?->producto_id,
                    'tipo_adquisicion'        => $linea['tipo_adquisicion'] ?? 'bien',
                    'municipio_id'            => $linea['municipio_id'] ?? null,
    ...
    {
        $factura = Factura::with(['lineas.retenciones', 'lineas.producto', 'contrato.proveedor', 'contrato.movirubros.rubro', 'contrato.itemcontratos.producto', 'contrato.itemcontratos.movirubro', 'contrato.itemcontratos.rubro'])->find($facturaId);
        ...
        foreach ($factura->lineas as $idx => $fl) {
            ...
            if ($esAjuste || $sinItemcontrato) {
                ...
                $this->lineas[] = [
                    ...
                    'unidad'                  => $item->unidad ?? ($fl->producto->tipo ?? ''),
                    'rubro'                   => $item->rubro->nombre_rubro ?? '—',
                    'uso'                     => $item->producto->uso->nombre_uso ?? ($fl->producto->uso->nombre_uso ?? '—'),
    ...
    public function importarXml(): void
    {
        ...
        // Auto-llenar cabecera si el XML trae datos
        if (!empty($datos['numero']) && empty($this->numero_factura)) {
            // Extraer solo la parte numérica del número de factura
            // El XML trae "001", "0001", etc.
        ...
        $buscarMatch = function(array $lineasXml, $itemcontratos) use ($normalizar) {
            ...
            foreach ($lineasXml as $lineaXml) {
                ...
                // 2) Contención mutua (el nombre del XML está contenido en el de BD o viceversa)
                if (!$match) {
                ...
                // 3) Matching por tokens: todas las palabras significativas (3+ chars) del más corto aparecen en el más largo
                if (!$match) {
        ...
        // Buscar matches y contar resultados
        $lineasImportadas = 0;
        $lineasPendientes = [];
        $totalXml = 0;
        $totalContrato = 0;
        ...
        foreach ($matches as ['linea' => $lineaXml, 'match' => $itemMatch]) {
            ...
            if ($itemMatch) {
                ...
                $this->lineas[] = [
                    ...
                    'unidad'                  => $itemMatch->unidad,
                    'rubro'                   => $itemMatch->rubro->nombre_rubro ?? '—',
                    'uso'                     => $itemMatch->producto->uso->nombre_uso ?? '—',
                    'cantidad'                => $cantidad,
                    'municipio_id'            => null,
                    'municipio_nombre'        => '',
                ...
                // Acumular para validación de discrepancia (valores con IVA)
        ...
        // Validar discrepancia de valores
