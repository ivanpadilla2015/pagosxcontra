...
new class extends Component
{
    ...
    #[Computed]
    public function totalRiesgos(): int
    {
        if (!$this->contratoId) {
            return 0;
        }

        return \App\Models\Riesgo::where('contrato_id', $this->contratoId)->count();
    }
    ...
    public function importarRiesgos(): void
    {
        $this->validate([
            'archivoImportacion' => ['required', 'file', 'mimes:xlsx,xls,csv', 'max:10240'],
        ...
        try {
            $import = new RiesgosImport($this->contratoId);
...
<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Importar Riesgos Masivos</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Carga masiva de riesgos desde archivos Excel (.xlsx, .xls o .csv) para un contrato específico.</p>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl border border-gray-200 dark:border-gray-700/60 p-6 mb-6">
        ...
        @if ($contratoEncontrado)
            <div class="mt-3 px-4 py-3 rounded-lg bg-violet-50 border border-violet-200 text-violet-700 dark:bg-violet-900/30 dark:border-violet-700 dark:text-violet-400">
                {{ $contratoInfo }}
                <span class="ml-2 text-xs font-semibold">({{ $this->totalRiesgos }} riesgos existentes)</span>
            </div>
        @endif
    </div>
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl border border-gray-200 dark:border-gray-700/60 p-6">
                <div class="flex items-center justify-between mb-4">
                    <div class="flex items-center">
                        ...
                        <div>
                            <h2 class="text-lg font-semibold text-gray-800 dark:text-gray-100">Importar Riesgos</h2>
                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ $this->totalRiesgos }} riesgos registrados en este contrato</p>
                        </div>
                    </div>
                </div>

                <p class="text-xs text-gray-500 dark:text-gray-400 mb-3">Columnas esperadas (con encabezado): <span class="font-medium">Tipo</span>, <span class="font-medium">Descripción</span>, <span class="font-medium">Tratamiento</span>, <span class="font-medium">Responsable</span>, <span class="font-medium">Periodicidad</span>.</p>

                <div wire:loading.remove wire:target="importarRiesgos">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Archivo Excel de riesgos</label>
                ...
                <div wire:loading wire:target="importarRiesgos" class="flex items-center text-sm text-violet-600 dark:text-violet-400 py-4">
                ...
                <button type="button" wire:click="importarRiesgos" wire:loading.attr="disabled" wire:target="importarRiesgos,archivoImportacion"
                ...
                <div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl border border-gray-200 dark:border-gray-700/60 p-6">
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center gap-3 p-3 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-700/60">
                            ...
                            <span class="text-sm text-emerald-700 dark:text-emerald-400">Riesgos creados</span>
                        </div>
        ...
        <div class="mt-6 bg-white dark:bg-gray-800 shadow-sm rounded-xl border border-gray-200 dark:border-gray-700/60 p-6">
            <div class="flex items-center justify-between mb-4">
                <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Descarga la plantilla con la estructura correcta para importar riesgos.</p>
                </div>
                <a href="{{ route('importar.riesgos.plantilla') }}" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-lg hover:bg-emerald-100 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-700/60 transition">
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700">
                            ...
                            <th class="px-4 py-3 text-left">Periodicidad</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        <tr class="bg-gray-50 dark:bg-gray-700/30">
                            <td class="px-4 py-3 text-gray-600 dark:text-gray-400">Estratégico</td>
                            <td class="px-4 py-3 text-gray-600 dark:text-gray-400">Retraso en la entrega de materiales por parte del proveedor.</td>
                            <td class="px-4 py-3 text-gray-600 dark:text-gray-400">Seguimiento semanal al estado de entrega y alerta temprana.</td>
                            <td class="px-4 py-3 text-gray-600 dark:text-gray-400">Carlos Méndez</td>
                            <td class="px-4 py-3 text-gray-600 dark:text-gray-400">Mensual</td>
                        </tr>
                        <tr>
                            <td class="px-4 py-3 text-gray-600 dark:text-gray-400">Operativo</td>
                            <td class="px-4 py-3 text-gray-600 dark:text-gray-400">Falla en los equipos suministrados durante la vigencia del contrato.</td>
                            <td class="px-4 py-3 text-gray-600 dark:text-gray-400">Garantía y reposición por parte del proveedor según cláusula contractual.</td>
                            <td class="px-4 py-3 text-gray-600 dark:text-gray-400">Laura Rodríguez</td>
                            <td class="px-4 py-3 text-gray-600 dark:text-gray-400">Trimestral</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div class="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700 dark:border-amber-700/60 dark:bg-amber-900/20 dark:text-amber-400">
                <strong>Nota:</strong> Los riesgos duplicados (misma descripción + tratamiento) se omiten automáticamente. Los campos Tipo y Periodicidad son opcionales.
            </div>
        </div>
    @endif
</div>
