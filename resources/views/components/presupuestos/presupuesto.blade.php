...
/**
 * Componente Volt (Livewire 4) para gestionar el CRUD de "Presupuestos".
 *
 * Archivo único que combina la lógica PHP (clase anónima que extiende Component)
 * y la vista Blade, tal como funciona Volt.
 */
new class extends Component
{
    ...
    /**
     * ID del presupuesto que se está editando.
     * null = modo "crear"; un entero = modo "editar".
     */
    public ?int $editingId = null;

    /** ID del presupuesto seleccionado para eliminar (guardado al confirmar). */
    public ?int $presupuestoToDeleteId = null;

    /** Nombre del presupuesto a eliminar, usado solo para mostrarlo en el modal. */
    public string $presupuestoToDeleteName = '';

    /** Campo del formulario: nombre del presupuesto. */
    public string $name = '';

    /** Campo del formulario: cargo del presupuesto. */
    public string $cargo = '';
    ...
    /**
     * Devuelve la lista paginada de presupuestos.
     ...
     *
     * @param int|null $id ID del presupuesto a editar. Si es null, es modo crear.
     */
    public function openModal(?int $id = null): void
    {
        ...
        // Si recibimos un ID, cargamos los datos del presupuesto a editar.
        if ($id) {
            $presupuesto = Presupuesto::findOrFail($id);
            $this->name = $presupuesto->name;
            $this->cargo = $presupuesto->cargo;
        }
     ...
     * Livewire resuelve el modelo Presupuesto automáticamente desde el ID
     * que se le pasa en el botón (wire:click="confirmDelete({{ $presupuesto->id }})").
     *
     * @param Presupuesto $presupuesto Modelo inyectado por Livewire.
     */
    public function confirmDelete(Presupuesto $presupuesto): void
    {
        $this->presupuestoToDeleteId = $presupuesto->id;
        $this->presupuestoToDeleteName = $presupuesto->name;
    ...
    public function closeDeleteModal(): void
    {
        ...
        $this->presupuestoToDeleteId = null;
        $this->presupuestoToDeleteName = '';
    }

    /**
     * Elimina definitivamente el presupuesto seleccionado y muestra un mensaje.
     * Usa findOrFail para lanzar 404 si el ID ya no existe.
     */
    public function delete(): void
    {
        Presupuesto::findOrFail($this->presupuestoToDeleteId)->delete();
    ...
    /**
     * Valida el formulario y crea o actualiza el presupuesto según corresponda.
    ...
    public function save(): void
    {
        ...
        if ($this->editingId) {
            ...
            Presupuesto::findOrFail($this->editingId)->update($data);
...
<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    {{-- Encabezado: título de la página y botón "Nuevo Presupuesto" --}}
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Presupuestos</h1>
        {{-- Abre el modal en modo crear (sin ID) --}}
        <button type="button" wire:click="openModal()" class="btn bg-white hover:bg-gray-100 text-gray-800 border border-gray-200">Nuevo Presupuesto</button>
    </div>
    ...
    {{-- Tabla con la lista paginada de presupuestos (usa la propiedad computada $this->presupuestos) --}}
    <div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl border border-gray-200 dark:border-gray-700/60">
        <table class="table-auto w-full">
            ...
            <tbody class="text-sm divide-y divide-gray-100 dark:divide-gray-700/60">
                {{-- Recorre los presupuestos; @empty muestra mensaje si no hay resultados --}}
                @forelse ($this->presupuestos as $presupuesto)
                    <tr>
                        <td class="px-4 py-3 text-gray-800 dark:text-gray-100">{{ $presupuesto->name }}</td>
                        <td class="px-4 py-3 text-gray-800 dark:text-gray-100">{{ $presupuesto->cargo }}</td>
                        <td class="px-4 py-3 text-right whitespace-nowrap">
                            {{-- Botón editar: abre el modal pasando el ID --}}
                            <button type="button" wire:click="openModal({{ $presupuesto->id }})" class="text-violet-500 hover:text-violet-600 mr-3" title="Editar">
                            ...
                            <button type="button" wire:click="confirmDelete({{ $presupuesto->id }})" class="text-rose-500 hover:text-rose-600" title="Eliminar">
                ...
                @empty
                    <tr>
                        <td colspan="3" class="px-4 py-6 text-center text-gray-500 dark:text-gray-400">No hay presupuestos registrados.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    @if ($modalOpen)
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60" wire:click="closeModal" wire:key="presupuesto-modal">
            <div class="w-full max-w-md bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6" wire:click.stop>
                ...
                <h2 class="text-lg font-bold text-gray-800 dark:text-gray-100 mb-4">{{ $editingId ? 'Editar Presupuesto' : 'Nuevo Presupuesto' }}</h2>
    ...
    @if ($deleteModalOpen)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60" wire:click="closeDeleteModal" wire:key="delete-modal">
            <div class="w-full max-w-md bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6" wire:click.stop>
                ...
                <h2 class="text-lg font-bold text-gray-800 dark:text-gray-100 mb-2 text-center">Eliminar Presupuesto</h2>
                <p class="text-sm text-gray-600 dark:text-gray-400 text-center mb-6">¿Estás seguro de que deseas eliminar <span class="font-semibold text-gray-800 dark:text-gray-100">{{ $presupuestoToDeleteName }}</span>? Esta acción no se puede deshacer.</p>
