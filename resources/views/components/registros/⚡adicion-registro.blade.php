...
new class extends Component
{
    ...
    // Movirubros individuales del contrato (Primer Registro y Adiciones existentes)
    ...
    // Fields for existing movirubro addition
    public $selected_movirubro_index = null;
    public $add_valor_rubro = 0;
    ...
    public function addDetalle()
    {
        ...
        $this->detalles[] = [
            ...
            'saldo_rubro' => $this->new_valor_rubro,
    ...
    public function addExistingRubro($index)
    {
        $this->validate([
            'add_valor_rubro' => 'required|numeric|min:1',
        ]);
        ...
        $this->detalles[] = [
            ...
            'valor_rubro' => $this->add_valor_rubro,
            'saldo_rubro' => $this->add_valor_rubro,
            'dependencia_afectacion' => $movirubro['dependencia_afectacion'],
    ...
    public function edit($id)
    {
        ...
        $this->detalles = $registro->movirubros->map(fn ($m) => [
            ...
            'registro_tipo' => $m->movirubroPadre?->registro?->tiporegistro?->name ?? 'Nuevo',
            'registro_numero' => $m->movirubroPadre?->registro?->numero_reg ?? '-',
            'es_nuevo' => $m->movirubro_padre_id ? false : true,
        ])->toArray();
    ...
    public function store()
    {
        ...
        $registro = Registro::create([
            ...
            'estado' => $this->estado,
            'tiporegistro_id' => 2, // Adicion
            'contrato_id' => $this->contrato_id,
        ]);

        foreach ($this->detalles as $detalle) {
            if ($detalle['es_nuevo']) {
                // New rubro - create new movirubro
                Movirubro::create([
                ...
                // Existing movirubro - create addition movirubro with reference to parent
                Movirubro::create([
                    'registro_id' => $registro->id,
                    'rubro_id' => $detalle['rubro_id'],
                    'valor_rubro' => $detalle['valor_rubro'],
        ...
        session()->flash('message', 'Adición de registro creada exitosamente.');
    ...
    public function update()
    {
        ...
        foreach ($registro->movirubros as $oldMovirubro) {
            if ($oldMovirubro->movirubro_padre_id) {
                Movirubro::where('id', $oldMovirubro->movirubro_padre_id)
                    ->decrement('saldo_rubro', $oldMovirubro->valor_rubro);
            }
        }
        ...
        session()->flash('message', 'Adición de registro actualizada exitosamente.');
        $this->showFormModal = false;
        $this->editing = false;
        $this->resetInputFields();
    }
    ...
    public function delete()
    {
        ...
        foreach ($registro->movirubros as $movirubro) {
            if ($movirubro->movirubro_padre_id) {
                Movirubro::where('id', $movirubro->movirubro_padre_id)
                    ->decrement('saldo_rubro', $movirubro->valor_rubro);
            }
        }
        ...
        session()->flash('message', 'Adición de registro eliminada exitosamente. Los saldos han sido restaurados.');
    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">Adición de Registros</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Gestión de adiciones a contratos</p>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-sm border border-gray-200 dark:border-gray-700">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-6 py-5 border-b border-gray-200 dark:border-gray-700">
                ...
                <x-button wire:click="create">
                    ...
                    <span class="ml-2">Crear Adición</span>
                </x-button>
            </div>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                    No se encontraron adiciones de registros.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            <div x-show="show"
                ...
                <div class="px-6 py-4 max-h-[85vh] overflow-y-auto">
                    <div class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">
                        {{ $editing ? 'Editar Adición' : 'Crear Nueva Adición' }}
                    </div>
                    ...
                    <div class="space-y-6">
                        {{-- Sección: Datos de la Adición --}}
                        <div class="border-l-4 border-blue-500 bg-blue-50/50 dark:bg-blue-900/10 rounded-r-lg p-4">
                            <h3 class="text-sm font-semibold text-blue-700 dark:text-blue-400 mb-3">Datos de la Adición</h3>
                                                    ...
                                                    <td class="px-4 py-2 text-center">
                                                        @if ($selected_movirubro_index === $i)
                                                            <div class="flex items-center gap-2 justify-center">
                                                                <input type="number" wire:model.live="add_valor_rubro" min="1" class="w-32 rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 text-sm" placeholder="Valor">
                                                                <button wire:click="addExistingRubro({{ $i }})" class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium text-green-600 bg-green-50 rounded hover:bg-green-100 dark:bg-green-900/20 dark:text-green-400 transition" title="Confirmar">
                                                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                                                                    </svg>
                                                                </button>
                                                                <button wire:click="$set('selected_movirubro_index', null); $set('add_valor_rubro', 0)" class="text-gray-400 hover:text-gray-600 transition" title="Cancelar">
                                                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        ...
                                                        @else
                                                            <button wire:click="$set('selected_movirubro_index', {{ $i }})" class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium text-blue-600 bg-blue-50 rounded hover:bg-blue-100 dark:bg-blue-900/20 dark:text-blue-400 transition" title="Adicionar a este movirubro">
                                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        {{-- Sección: Agregar Nuevo Rubro --}}
                        <div class="border-l-4 border-green-500 bg-green-50/50 dark:bg-green-900/10 rounded-r-lg p-4">
                            <h3 class="text-sm font-semibold text-green-700 dark:text-green-400 mb-3">Agregar Nuevo Rubro</h3>
                            <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-4">
                                ...
                                <div class="flex items-end">
                                    <x-button wire:click="addDetalle" class="bg-green-600 hover:bg-green-700">
                                        <svg class="w-4 h-4 fill-current shrink-0" viewBox="0 0 16 16">
                                            <path d="M15 7H9V1c0-.6-.4-1-1-1S7 .4 7 1v6H1c-.6 0-1 .4-1 1s.4 1 1 1h6v6c0 .6.4 1 1 1s1-.4 1-1V9h6c.6 0 1-.4 1-1s-.4-1-1-1z" />
                                        </svg>
                        ...
                        {{-- Sección: Detalle de Rubros a Adicionar --}}
                        <div class="border-l-4 border-violet-500 bg-violet-50/50 dark:bg-violet-900/10 rounded-r-lg p-4">
                            <h3 class="text-sm font-semibold text-violet-700 dark:text-violet-400 mb-3">Detalle de Adición</h3>
                                    ...
                                    <table class="w-full text-sm">
                                        <thead>
                                            <tr class="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700">
                                                ...
                                                <th class="px-4 py-2 text-right">Valor Adición</th>
                                                <th class="px-4 py-2 text-left">Dependencia</th>
                                                <th class="px-4 py-2 text-center">Acción</th>
                                            </tr>
                                        </thead>
                                                    ...
                                                    <td class="px-4 py-2">
                                                        ...
                                                        @else
                                                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400">
                                                                [{{ $detalle['registro_tipo'] }}] N°{{ $detalle['registro_numero'] }}
                                                            </span>
                                                        @endif
                                                    </td>
                                                    <td class="px-4 py-2">{{ $detalle['codigo_rubro'] }}</td>
                                                    <td class="px-4 py-2 font-medium">{{ $detalle['nombre_rubro'] }}</td>
                                                    <td class="px-4 py-2 text-right text-green-600 dark:text-green-400">${{ number_format($detalle['valor_rubro'], 2, ',', '.') }}</td>
                            ...
                            @else
                                <div class="text-center py-4 text-gray-500 dark:text-gray-400">
                                    No hay rubros agregados. Use los formularios superiores para agregar detalles.
                                </div>
                            @endif
                        </div>
                    </div>
            ...
            <div x-show="show"
                ...
                <div class="p-6">
                    ...
                    <div class="text-center">
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">¿Está seguro que desea eliminar esta adición? Esta acción no se puede deshacer.</p>
