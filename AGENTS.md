...
- **Importar facturas desde XML DIAN** (`app/Services/XmlFacturaReader.php`): parser de XMLs de facturación electrónica colombiana (UBL 2.1). Soporta `AttachedDocument` (con CDATA interno) e `Invoice`/`CreditNote` directo. Usa regex para extraer datos del CDATA (SimpleXML no puede parsearlo por firmas DIAN). Extrae: número, fecha, líneas (nombre, cantidad, precio, IVA). Implementado en `facturar.blade.php` y `facturacion.blade.php` con botón "Importar desde XML" visible solo al tener contrato cargado. Trait `WithFileUploads` en ambos componentes
- **Matching fuzzy de productos XML**: al importar XML, los nombres se comparan con 3 niveles: (1) exacto (`strtolower(trim())`), (2) contención mutua (si un nombre contiene al otro), (3) por tokens (todas las palabras de 3+ caracteres del XML aparecen en el nombre de BD). Prioridad: exacto > contención > tokens
- **Validación de discrepancia XML vs contrato**: al importar XML, si el total calculado del XML difiere del total del contrato, se muestra warning con la diferencia
- **Módulo de Facturación Sencilla** (`/contratos/facturacion-sencilla`): crea facturas con producto genérico "Facturación Sencilla" sin itemcontratos del contrato. Flujo: buscar contrato → seleccionar rubro (movirubro) y uso → ingresar valor total con IVA → calcular retenciones → guardar. Producto genérico creado por `ProductoGenericoSeeder`
- **Edición de Facturación Sencilla**: ruta `facturacion-sencilla/{id?}` con soporte de edición. `cargarFactura()` carga movirubro_id desde `factura_lineas.movirubro_id`, fallback a `facturas.movirubro_id`. Uso se carga desde `factura_lineas.uso_id`. Retenciones se recalculan en `previewRetenciones` (computed property)
- **Movirubro_id y uso_id en factura_lineas**: columnas nullable en `factura_lineas` para persistir el rubro y uso seleccionados en facturación sencilla. Permite recuperar estos datos al editar. Modelo `FacturaLinea` actualizado con fillable y relación `movirubro()`
- **Movirubro_id en facturas**: columna nullable en `facturas` como respaldo para facturación sencilla. Modelo `Factura` actualizado con fillable
- **Facturas-lista redirige sencillas**: el botón editar en `facturas-lista` detecta `$factura->tipo === 'sencilla'` y redirige a `facturacion.sencilla` en vez de `facturacion` o `facturar`
- **Soporte de facturas sencillas en pagos**: `pago-crear` y `pago-editar` ahora agrupan por `$linea->movirubro_id ?? $linea->itemcontrato->movirubro_id`. Rubro se obtiene de `$linea->movirubro?->rubro`. Confirmación de pago descuenta saldos correctamente para facturas sin itemcontrato
...
77. **Auto-scroll en alertas de pagos**: en `pago-crear.blade.php` y `pago-editar.blade.php`, los divs de `session('error')` y `session('message')` tienen `x-init="$el.scrollIntoView({behavior:'smooth', block:'center'})"` para que al aparecer un error (ej: "Saldo insuficiente") la página haga scroll automático hacia la alerta
78. **Modal de contratos sin cierre al hacer clic fuera**: en `contrato.blade.php`, el backdrop (`fixed inset-0 bg-gray-900/60`) es un div separado del contenido (hermano, no padre). No tiene handler de clic. Solo se cierra con el botón "Cancelar". Se eliminó `wire:click` y `wire:click.stop` del backdrop/formulario. Aplica tanto al modal de crear/editar como al de eliminar
79. **Columna `valor_poliza_asegurado` ampliada**: migración `2026_09_08_143749_increase_valor_poliza_asegurado_column_in_contratos_table` cambia la columna de `decimal(10,2)` (máx $99,999,999.99) a `decimal(16,2)` (máx $999,999,999,999,999.99) para soportar valores más grandes
80. **Importar facturas desde XML DIAN**: parser `XmlFacturaReader` usa regex (SimpleXML no parsea el CDATA interno de `AttachedDocument` por firmas DIAN). Botón "Importar desde XML" visible solo con contrato cargado. Trait `WithFileUploads` para manejo de archivos. Matching fuzzy de productos: exacto → contención mutua → tokens (palabras de 3+ chars). Valores unitarios siempre del contrato (no del XML). Warning si totales difieren
81. **CDATA de DIAN no parsea con SimpleXML**: el `AttachedDocument` contiene el `Invoice`/`CreditNote` dentro de `cbc:Description` como CDATA. SimpleXML retorna NULL sin errores al parsearlo (problema conocido con firmas digitales). Se usa DOMDocument para validación + regex para extracción de datos. Tags en el CDATA usan prefijos de namespace (`cac:CreditNoteLine`, `cbc:ID`, etc.)
82. **Facturación Sencilla: ruta con `{id?}`**: `facturacion-sencilla/{id?}` permite creación y edición. El componente `facturacion-sencilla.blade.php` tiene `mount(?int $id = null)` que carga factura existente para editar
83. **Edición de sencillas por componente dedicado**: las facturas tipo `sencilla` se editan en `facturacion-sencilla`, NO en `facturacion` ni `facturar`. `facturas-lista` redirige según `$factura->tipo === 'sencilla'` a `route('facturacion.sencilla', $id)`
84. **`movirubro_id` y `uso_id` en factura_lineas**: columnas nullable para persistir rubro y uso de facturación sencilla (que no usa itemcontratos). Fallback en `cargarFactura`: `$linea->movirubro_id ?? $factura->movirubro_id`
