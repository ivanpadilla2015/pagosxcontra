...
- **Creación masiva de informes faltantes**: crea informes con `total_info=0`, `saldo_viene` acumulado, `%` calculado, y fecha = último día del mes (ajustado por fin de semana). Mueve pagos al consecutivo del último informe creado
- **Cálculo de % cumplimiento**: `ejecutado = saldo_viene + total_info`, `% = ejecutado / valorTotal * 100`. NO usa saldo del contrato (ya refleja pagos confirmados)
- **`saldo_viene` en informes**: suma de `total_info` de todos los informes anteriores. Se persiste en BD para consulta histórica
- **Delete de informes**: botón visible en todos, pero solo permite eliminar el último consecutivo (compara `cansecu_infor` con `contrato.cansecu_infor`). Al eliminar, se borran primero las `informeobligaciones` e `informeriesgos` asociados (no hay cascade en FK)
...
40. **Delete de informes**: botón visible en todos los informes, pero `confirmDelete()` solo permite eliminar el último consecutivo (compara `cansecu_infor` con `contrato.cansecu_infor`). Al eliminar, se borran primero las `informeobligaciones` e `informeriesgos` asociados (no hay cascade en FK)
